...
class PatchTSTEncoder(nn.Module):
    def __init__(self,c_in=1,num_patch=10, patch_len=64,n_layers=3,d_model=128,n_heads=16,shared_embedding=True,d_ff=256,
    ...
    def forward(self, x:Tensor, mask=None):
        ...
        if not self.shared_embedding:
            ...
            x=torch.stack(x_out,dim=2) ## (bs,num_patch,d_model,n_vars)
...
## to convert the raw timeseries into patched timeseries using sliding window
...
def padding_stride(x:Tensor,p=None,s=None):
    x=x.view(1,-1)
    ##print(x.shape)
